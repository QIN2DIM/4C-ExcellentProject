...
# 版本控制
def version_control(mode='', bash_name='国赛', _init=True) -> str:
    """
    ...
    :param mode: str 调用模式
        ...
        'new':获得更新版本的文件绝对路径
        'num':获得最新版本文件的版本号
    :param bash_name: str 文件名关键词字段
        系统文件强制关键词为”国赛“ ”合成“，分别表示爬虫采集的文件，以及信息表合成文件
    :param _init: 系统迭代参数，请勿改动
    :return:
    """

    try:
        # 取出对应BASE_NAME的最新版本文件
        out_dir_version = '{}_{}_{}.csv'.format(BASE_NAME, bash_name, sorted(
            [int(i.split('.csv')[0].split('_')[-1]) for i in os.listdir(ROOT_DIR_DATABASE) if bash_name in i])[-1])
    except IndexError:
    ...
    if mode == 'fn':
        return '{}/{}'.format(ROOT_DIR_DATABASE, out_dir_version)
...
# 赛区官网
HOME_PAGE = 'http://2020.jsjds.com.cn'
...
# 错误日志路径，记录采集异常并捕获链接
LOG_PATH = join(ROOT_DIR_DATABASE, 'error_log.txt')

# id源表文件位置，请勿改动源文件
ID_FP = join(ROOT_DIR_DATABASE, 'id_set_01.csv')
...
COMP_TABLE_FP = version_control('fn', bash_name='合成')

# 脚本核心组件，请勿挪动删除
COMP_DATABASE = join(ROOT_DIR_DATABASE, 'cnjsj_base.csv')
...
CHROME_CODE_PATH = join(ROOT_DIR_PROJECT, '/middleware/chromedriver.exe')

# :BACKUP_OUT_PATH:MTH离线网页文件的存储文件夹路径
BACKUP_PATH = join(ROOT_DIR_DATABASE, 'backup')

BACKUP_CASHED_FILE = [fp_ for fp_ in os.listdir(BACKUP_PATH) if '.mhtml' in fp_]
